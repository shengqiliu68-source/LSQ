...
def save_image(source: Path, target: Path):
    with Image.open(source) as image:
        image = ImageOps.exif_transpose(image).convert("RGB")
        image = ImageOps.fit(image, (1400, 1050), method=Image.Resampling.LANCZOS)
...
def main():
    ...
    for number, (slug, name, category, indices, colors, contents) in enumerate(GROUPS, 1):
        ...
        cases.append({
            ...
            "cover": f"/cases/{slug}/cover.webp", "gallery": gallery, "detail": "",
        })
